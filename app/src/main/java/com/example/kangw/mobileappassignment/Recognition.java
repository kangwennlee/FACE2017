...
public class Recognition {
    ...
    private static void step1_UploadImages() throws IOException,
        ...
        //for every person in peoples
        for (File person : peoples) {
            //get the file name
            String objectId = person.getName();
            //add requestparms + objectId
            String requestParams = "?train=manual&objectType=person&objectId="
                    + objectId;
            //for every image that person have
            for (File image : person.listFiles()) {
                //if the file contains data and is not hidden
                if (image.isFile() && !image.isHidden()) {
                    //encode base url + image link in HTML format and also requestParams
                    final String api = BASE_URL + "image/"
                            + URLEncoder.encode(image.getName(), "UTF-8")
                            + requestParams;
                    //Files.getCanonicalPath - Returns the canonical pathname string of this abstract pathname.
